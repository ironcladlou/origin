...
func (r *REST) Get(ctx kapi.Context, name string, opts runtime.Object) (runtime.Object, error) {
	...
	case deployapi.DeploymentStatusNew, deployapi.DeploymentStatusPending:
		...
		if err := waitForDeployment(r.DeploymentGetter, target, r.Timeout); err != nil {
			return nil, err
		}
...
// waitForDeployment waits until the passed deployment is Running, Complete, or Failed
func waitForDeployment(c kclient.ReplicationControllersNamespacer, deployment *kapi.ReplicationController, timeout time.Duration) error {
	fieldSelector := fields.Set{"metadata.name": deployment.Name}.AsSelector()
	w, err := c.ReplicationControllers(deployment.Namespace).Watch(labels.Everything(), fieldSelector, deployment.ResourceVersion)
	if err != nil {
		return err
	}
	defer w.Stop()
	...
	go func(ch <-chan watch.Event) {
		...
		for event := range ch {
			...
			if !ok {
				errchan <- fmt.Errorf("event object is not a deployment: %#v", event.Object)
				break
			}
			switch deployutil.DeploymentStatusFor(obj) {
			case deployapi.DeploymentStatusRunning, deployapi.DeploymentStatusComplete, deployapi.DeploymentStatusFailed:
				done <- struct{}{}
				break
			}
		}
	...
	case <-time.After(timeout):
		return errors.NewTimeoutError(fmt.Sprintf("timed out waiting for deployment %s", deployutil.LabelForDeployment(deployment)), 1)
	}
}
